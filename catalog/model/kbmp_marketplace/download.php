<?php
class ModelKbmpmarketplaceDownload extends Model {
	public function addDownload($data,$seller_id) {
		...
		$download_id = $this->db->getLastId();
                $this->db->query("INSERT INTO `" . DB_PREFIX . "kb_mp_seller_downloads` SET seller_id = '" . (int)$seller_id . "', download_id = '" . (int)$download_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', date_added = NOW(), date_updated = NOW()");
	...
	public function getDownloads($data = array(), $seller_id) {
		$sql = "SELECT * FROM " . DB_PREFIX . "download d LEFT JOIN " . DB_PREFIX . "download_description dd ON (d.download_id = dd.download_id)"
                        . " LEFT JOIN ".DB_PREFIX."kb_mp_seller_downloads as sd ON sd.download_id = d.download_id "
                        . "WHERE sd.seller_id = '".(int)$seller_id."' AND sd.seller_id IS NOT null AND dd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
